...
async function showDetail(c) {
  ...
  box.innerHTML = `
    ...
    <div class="detail-top">
      <img class="detail-flag" src="${c.flags.png}" onerror="this.src='${c.flags.svg}'">
      <div class="detail-info-table">
        <p>Native Name: ${getNative(c.name)}</p>
        <p>Capital: ${getCapital(c.capital)}</p>
        <p>Population: ${c.population}</p>
        <p>Region: ${c.region}</p>
        <p>Sub-region: ${c.subregion}</p>
        <p>Area: ${c.area} Km²</p>
        <p>Country Code: ${getCodeNum(c.idd)}</p>
        <p>Languages: ${getLanguages(c.languages)}</p>
        <p>Currencies: ${getCurrency(c.currencies)}</p>
  ...
  for (let b of borders) {
    ...
    if (n && n.flags) {
      ...
      img.src = n.flags.png || n.flags.svg;
